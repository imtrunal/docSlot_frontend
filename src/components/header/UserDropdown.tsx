...
export default function UserDropdown() {
  const [isOpenu, setIsOpen] = useState(false);
  ...
  if (role == "SUPER ADMIN") {
    ...
    name = userName?.trim().split(" ")[userName?.trim().split(" ").length - 2];
  }
  ...
  const GetClinicProfile = async () => {
    ...
    const res = await fetch(`${url}/clinic/profile`, {
      headers: {
    ...
    setFormData({
      ...
      ownerMobile: rawData.ownerMobile || "",
      password: rawData.password || "",
      address: rawData.address,
      loginNumber: rawData.loginNumber,
      city: rawData.city,
      state: rawData.state,
  ...
  const handleSave = async () => {
    ...
    const response = await fetch(`${url}/clinic/profile`, {
      method: "PUT",
      headers: {
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-10 w-11">
          <img src="./images/user/user.png" alt="User" className="w-40" />
        </span>
      ...
      <Dropdown
        ...
        className="fixed z-999999 userDropdown right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark"
      >
        <div>
          ...
          {role !== "SUPER ADMIN" && (
            <span className="block  text-gray-300 text-sm">{clinicName}</span>
          )}
        </div>
